...
def escalator_controller(stream, config, model, tracker, max_count, soft_limit):
    people = []
    gate_closed = False
    while stream.isOpened():
        ...
        for result in results:
            ...
            if is_in_limits(config["limits_in"], cx, cy, 15) and people.count(_id) == 0:
                people.append(_id)
            if (
                is_in_limits(config["limits_out"], cx, cy, 15)
                and people.count(_id) == 1
            ):
                people.remove(_id)
    ...
    def __init__(self, image):
        ...
        def get_pos(event):
            ...
            if len(self.points) == 8:
                ...
                cv2.fillPoly(mask, [mask_points.astype(np.int32)], (255, 255, 255))
                config["mask"] = mask
                ...
                soft_limit = 1
                self.close()
                escalator_controller(
                    stream, config, model, tracker, max_count, soft_limit
                )

                stream.release()

        self.frame.mousePressEvent = get_pos
